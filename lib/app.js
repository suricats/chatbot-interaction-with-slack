...
slackbot.on('start', function() {
  ...
  bot.dialog('/postSlack', [
      function (session) {
        ...
        slackbot.postMessageToChannel(config.slackChannel, '@' + userId + ' : ' + session.message.text, params).then(function(data){
          ...
          if(!session.userData.profile){
            ...
            session.userData.profile = { id : userId };
          }
        });
      }
  ]);
